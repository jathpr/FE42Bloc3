...
const getGenres = (array: Movie[]) => {
    const arrayOfAllGenres = array.map(movie => movie.genre)
    const allGenres = []
    const genres = allGenres.concat(...arrayOfAllGenres)
    const resultArray = Array.from(new Set(genres))
    return resultArray
}
...
const sortByImdbRating = (array: Movie[]) => {
    const sortedArray = array.sort((a, b) => b.imdbRating - a.imdbRating)
    return sortedArray
}
console.log(sortByImdbRating(movies));

// 4. Новый массив, где объекты фильмов состояни из полей id, title, released, plot
const newMoviesArray = (array: Movie[]) => array.map(movie => ({id: movie.id, title: movie.title, released: movie.released, plot: movie.plot}))
console.log(newMoviesArray(movies));

// 5. Функция принимает массив и число. Число = году выхода фильма
const filteredMoviesByYear = (array: Movie[], num: number) => array.filter(movie => movie.year === num)
