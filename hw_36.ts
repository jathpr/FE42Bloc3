...
const getAllGenres = (movies: Movie[]) => {
	const allGenresArrRepeated = movies.reduce((allGenres: string[], movie) => [...allGenres, ...movie.genre], [])
	const allGenresObj = new Set(allGenresArrRepeated)
...
const getAllActors = (movies: Movie[]) => {
	const allActorsArrRepeated = movies.reduce((allActors: string[], movie) => [...allActors, ...movie.actors], [])
	const allActorsObj = new Set(allActorsArrRepeated)
...
const sortMoviesByRating = (movies: Movie[]) => {
	const copiedMovies = [...movies]
	const sortedMovies = copiedMovies.sort((movie1, movie2) => movie2.imdbRating - movie1.imdbRating)
	return sortedMovies
}
...
const createMoviePreview = (movies: Movie[]) => {
	const moviesPreview = movies.map((movie) => ({ id: movie.id, title: movie.title, released: movie.released, plot: movie.plot }))
...
const findMoviesByYear = (movies: Movie[], year: number) => {
	const moviesOfSelectedYear = movies.filter((movie) => movie.year === year)
...
const findMoviesByPartTitle = (movies: Movie[], incomplTitle: string) => {
	const moviesWithFittingTitle = movies.filter((movie) => movie.title.toLowerCase().includes(incomplTitle.toLowerCase()))
...
const findMoviesByPartTitleorPlot = (movies: Movie[], line: string) => {
	const moviesWithFittingTitle = movies.filter((movie) => movie.title.toLowerCase().includes(line.toLowerCase()) || movie.plot.toLowerCase().includes(line.toLowerCase()))
	return moviesWithFittingTitle
}
...
const getMoviesByPropValue = (movies: Movie[], property: string, value: string | number) => {
	const moviesWithReqPropValue = movies.filter((movie) => movie[property] === value)
